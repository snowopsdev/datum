...
Fix commit: `11e81b663da522c1cdb2a6780e9edd66150eac80`. Refresh each newly observed settled run once; unchanged settled polling does not refresh repeatedly. Focused run-bar and polling tests8/8 passed; CMS typecheck and scoped lint passed.
...
Broader regression at the fix: populated-workspace suite initially15/16 passed because the dashboard test assumed `/admin` even after the documented redirect to Content. Harness maintenance commit `39cc8e0` now checks the exact expected heading for each valid destination; this is excluded from app defect counts. The full suite then16/16 passed in19.7s, scoped lint passed.

## Expansion

## Keyboard candidate: template radio controls ignore arrow keys

Severity: minor; canonical ID requested from coordinator. At the base, select a template on New content, focus its `role=radio` button and press ArrowRight. The next template is neither selected nor focused; all template buttons are in the Tab sequence. The declared radiogroup lacks its expected keyboard interaction.
...
- No published branches/PRs from this worker; coordinator owns publication and integration.
