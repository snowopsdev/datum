...
**A decision only ever describes one draft.** Both gates above fire on status *transitions*, which left the other half of the surface open: editing the body of an already-`verified` article is not a transition, so it used to keep a `PASS` earned by text nobody could read any more — and that same `PASS` could later authorise another move to `verified`. A third hook, `invalidateStaleInformationGain`, closes it. When an update changes any field the scoring stage actually reads — `title`, `body`, `keyword`, `research.snapshot`, `research.queryCluster`, `research.facets`, listed as `SCORED_CONTENT_FIELDS` beside the stage that defines them — it nulls the whole `informationGain` group. Fields the scorer never sees (`titleTag`, `metaDescription`, the `og*` fields, `faqItems`, and the rest of the `research` group) are deliberately not on that list: structural QA checks them, but editing one cannot make a scorecard wrong.

An article sitting at `verified` also goes back to **`drafted`** when that happens, unless the same edit was already sending it backwards. Clearing the decision alone would not be enough — `gateVerifiedStatus` never fires on a re-save at `verified`, and nothing gates `verified → approved` — so unscored text could still walk to publication, just without a verdict beside it. `drafted` is the status the pipeline converges from on its own: QA picks it up, and a pass there feeds it straight back into scoring at `qa_passed`. An article already past `verified` (`approved`, `published`) keeps its status and loses only the stale summary; demoting a published article behind the editor's back would be worse than the staleness. The hook runs *first* in the `beforeChange` chain, because `gateVerifiedStatus` has to see the clearance rather than the `PASS` it replaced. Clearing an `access.update: () => false` group from a collection hook works because Payload enforces field access during the beforeValidate-fields phase, which runs before collection `beforeChange`; that same ordering is why the hook always sees a complete document and a partial REST update compares equal on everything it did not send.
...
1. It reads the run the article's **current** `informationGain.run` pointer names — not merely the newest row filed against the article — and turns its `reasons` into `revisionNotes`, one line per reason: `- [CODE] message`. With no pointer it falls back to the QA failure lines, and a reviewer note, when given, is appended as `Reviewer note: …`. The pointer is what makes the notes describe the draft being replaced: run rows are immutable and are never deleted, so an article that has already been regenerated once still has that first run sitting behind it, and taking the newest row would aim the second regeneration at a problem the current draft may not even have. The cost is that the two-step route above (**Send back**, then **Regenerate**) no longer inherits the blocking run's reasons — Send back nulls the summary, so Regenerate sees no pointer — and the reviewer should put what they want fixed in the Regenerate note.
...
Advancing the article from there regenerates the draft with those notes rendered verbatim into the prompt as `# Revision notes (previous attempt)`, ending with "Fix these before anything else" (see Gap-fed generation above), then re-runs QA and scoring. `pipeline:run` is one way to do that, but no longer the only one: the board's **Run** / **Run selected** and the article page's **Run next stage** queue the same article as a `pipeline-runs` row, processed as a native Payload `content-run` task. The research stage is skipped, so the article keeps the corpus snapshot it was judged against and the new draft is scored against the same baseline — which is the only way the second scorecard is comparable to the first.
