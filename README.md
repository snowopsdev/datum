...
Everything that decides how Datum writes lives under **Setup** in the admin nav, and a run reads it when it starts. The template is the exception: each stage re-reads the one attached to the article it is working on, so editing a template mid-run can reach the later stages.
