...
`/admin` opens on the setup checklist and keeps showing it until the four required steps are done and the workspace holds at least one piece; the three recommended steps never block it. If you would rather look around first, **Start with the demo workspace** fills every required and recommended step but Templates and Models from fixtures in one click; it only ever fills what is still blank, so pressing it twice is safe. Templates are seeded. Missing live-provider keys show as a banner for whoever deploys — they never block an editor.

Then **New content** (`/admin/ops/new`) shows the same blockers as a notice if anything required is still missing; otherwise pick the kind of piece (a template card), say what it is about — suggested topics from Ahrefs, or a keyword you already know — and create it. Research starts on its own; the piece opens; when research is done a **brief** appears with the angle, audience and sections. Edit it, approve it, and Datum writes the draft, runs the checks and scores it. **Content** (`/admin/ops/content`) is the one list: every piece on a five-step stepper (Research → Brief → Writing → Review → Publish), who it is waiting on, and the one thing to do next. A stalled piece — waiting on a person rather than a run — carries a **Run next stage** button on its own page and a **Run** button on the board; select several and **Run selected** advances them together. Each of those queues the article as a `pipeline-runs` row, executed as a native Payload `content-run` task, so `npm run jobs:run --workspace cms` (or dev's in-process worker) is what actually processes it; a bar at the bottom of every admin page shows what the model is doing while a run is in flight. `pipeline:run` reaches the same stages directly from the CLI, outside that queue, and is no longer the only way to move a stalled piece forward. Live mode asks for a paid-provider confirmation before the expensive half starts.
...
For live API calls, set `MOCK_MODE=false` and provide the key for each chosen model's provider (`ANTHROPIC_API_KEY` or `OPENAI_API_KEY`). Ahrefs also needs `AHREFS_API_KEY` plus a target domain and competitors, which come from the **Workspace** setup page (`/admin/ops/setup/workspace`); `TARGET_DOMAIN` and `COMPETITOR_DOMAINS` are its fallback. See [`.env.example`](.env.example).
...
| `TARGET_DOMAIN` | Fallback for the Workspace global's target domain. The admin field wins; a mock run falls back again to a demo domain. The `.env.example` placeholder (`example.com`) is treated as unset rather than a real domain. |
| `COMPETITOR_DOMAINS` | Comma-separated fallback for the Workspace global's competitor list, same rule. The `.env.example` placeholders (`competitor-a.com`, `competitor-b.com`) are likewise treated as unset. |
...
Read only by the test suites, never by the running app. Set these in your shell, not in a committed `.env`.

| Variable | Purpose |
| --- | --- |
| `DISABLE_DEV_PUSH` | `true` to turn off Payload's dev schema push, which `next dev` otherwise runs on boot. Playwright's `test:e2e` script sets it automatically so its parallel workers don't race each other pushing the same schema; set it yourself in the shell that runs `npm run dev` when you point Playwright at that server via `TEST_BASE_URL` instead. |
| `TEST_BASE_URL` | Base URL Playwright drives instead of starting its own dev server — e.g. `http://127.0.0.1:3000` to test against a server you already started with `npm run dev`. Unset, Playwright starts its own on `TEST_PORT`. |
| `TEST_PORT` | Port for the dev server Playwright starts when `TEST_BASE_URL` is unset (default `3101`). |
