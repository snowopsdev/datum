...
type Section = { label: string; links: { label: string; href: string }[]; collapsed?: boolean }
      ...
      {SECTIONS.map((section) => {
        const links = section.links.map((link) => (
          <Link
            ...
            className={`datum-ops-nav__link${isActive(link.href) ? ' is-active' : ''}`}
            href={link.href}
            key={link.href}
            prefetch={false}
          >
        ...
        if (section.collapsed) {
          ...
          const open = section.links.some((l) => isActive(l.href))
          return (
